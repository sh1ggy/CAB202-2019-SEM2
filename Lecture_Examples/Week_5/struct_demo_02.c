 ...
 *  p   - The address of an instance of struct point which will 
 *        be populated such that p->ok is true if an only if two 
 *        valid floating point values were scanned, and both are 
 *        between 0.0 and 1.0 inclusive. The values scnned will be
 *        stored in p->x and p->y.  
 */
 ...
 * Parameters:
 *  f   The address of a FILE object connected to the stream.
 *  p   The address of a point to be sent to the stream.
 */
