...
struct point read_point(FILE *f)
{
    ...
    int fields_scanned = fscanf(f, "%lf %lf", &result.x, &result.y);
    result.ok = (fields_scanned == 2) && (result.x >= 0) && //
...
void draw_point(struct point p, int ch)
{
    if (p.ok)
    {
        ...
        draw_char(x, y, ch);
#if DB
        fprintf(stderr, "drawing point (%f,%f) at (%d,%d)\n", p.x, p.y, x, y);
#endif
    }
}
